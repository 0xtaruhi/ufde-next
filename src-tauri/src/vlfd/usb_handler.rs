use super::constants::{DW_PID, DW_VID};
...
impl UsbHandler {
    ...
    pub fn open(&mut self) -> DeviceResult<()> {
        ...
        let handle = unsafe {
            libusb_ffi::libusb_open_device_with_vid_pid(std::ptr::null_mut(), DW_VID, DW_PID)
        };
        ...
        unsafe {
            ...
            if result < 0 {
                ...
                return Err(format!("device open failed (error {result}, {error_str})"));
            };
