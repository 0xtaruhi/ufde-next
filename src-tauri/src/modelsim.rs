...
fn preprocess_netlist(
    input_path: &std::path::Path,
    output_path: &std::path::Path,
) -> Result<(), String> {
    let input_file =
        std::fs::File::open(input_path).map_err(|e| format!("failed to open netlist: {}", e))?;
    ...
    let duplicate_base_re =
        Regex::new(r#"(\d+'h)(\d+'h)"#).map_err(|e| format!("regex error: {}", e))?;
    ...
    for line in reader.lines() {
        ...
        let processed_line = if let Some(caps) = re.captures(&line) {
            ...
            let is_bram_param = bram_params.contains(&param_name_lower.as_str());

            if is_bram_instance && is_bram_param {
                if param_name_lower.starts_with("init_") {
                    format!(
                        "defparam {}.{} = 256'h{};",
                        instance_path, param_name_lower, value
                    )
                } else {
                    format!(
                        "defparam {}.{} = \"{}\";",
...
pub async fn run_modelsim_simulation(args: SimulationArgs) -> Result<String, String> {
    ...
    let simlib_file =
        if args.netlist_path.ends_with("_gate.v") || args.netlist_path.ends_with("_map.v") {
            "MAPPING_SIMLIB.v"
    ...
    if work_dir.exists() {
        std::fs::remove_dir_all(&work_dir)
            .map_err(|e| format!("failed to remove old work directory: {}", e))?;
    }
    ...
    if default_custom_simlib.exists() {
        ...
        return Err(format!(
            "custom_simlib.v not found at: {}",
            default_custom_simlib.display()
        ));
    }
