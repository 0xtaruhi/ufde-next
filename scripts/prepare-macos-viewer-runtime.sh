...
while [[ $# -gt 0 ]]; do
  case "$1" in
    --target)
      ...
      ;;
    -h|--help)
      echo "usage: prepare-macos-viewer-runtime.sh [--target aarch64-apple-darwin]" >&2
      exit 0
      ;;
    *)
...
if [[ "$(uname -s)" != "Darwin" ]]; then
  echo "This script prepares macOS viewer runtime resources and must run on macOS." >&2
  exit 1
fi

if [[ "$TARGET_TRIPLE" != "aarch64-apple-darwin" ]]; then
  echo "macOS viewer runtime resources are currently committed for aarch64-apple-darwin only." >&2
  echo "Provide matching macOS runtime resources before building $TARGET_TRIPLE." >&2
  exit 1
fi

mkdir -p "$MACOS_BIN_DIR" "$MACOS_LIB_DIR" "$MACOS_PLUGIN_DIR"
